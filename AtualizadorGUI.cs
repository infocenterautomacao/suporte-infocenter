...
namespace AtualizadorGUI
{
	...
	internal class FormPrincipal : Form
	{
		...
		private void ConstruirPainelRotinas()
		{
			...
			chkRotinas.Items.Add("Duplicador de instância de Firebird e modificação de porta", false);
		...
		private void BtnExecutarRotinas_Click(object sender, EventArgs e)
		{
			...
			foreach (object item in chkRotinas.CheckedItems)
			{
				...
				else if (rotina.Contains("Duplicador de inst"))
				{
					RodarScriptEmbutido("Duplicar_Firebird.bat", ObterBatDuplicarFirebird());
				}
			}
		}
		...
		private string ObterBatDuplicarFirebird() {
			return @"@echo off
					...
					using (WebResponse respGe = await reqGe.GetResponseAsync())
					{
						...
						listaDownloadsDisponiveis.Add(new SistemaDownload {
							...
							Exibicao = sizeStr + (dateStr != "" ? " (" + dateStr + ")" : "")
								...
								listaDownloadsDisponiveis.Add(new SistemaDownload
				{
					Nome = "Firebird-UpSystem",
					ArquivoNome = "Firebird-5.0.3.1683-0-windows-x64.exe",
					UrlDownload = "https://raw.githubusercontent.com/infocenterautomacao/suporte-infocenter/main/dependencias/Firebird-5.0.3.1683-0-windows-x64.exe",
					Exibicao = "v5.0.3"
				});
